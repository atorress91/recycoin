...
let PrintService = class PrintService {
    constructor(http, affiliateService, toastr) {
        this.http = http;
    ...
    downloadInvoice(invoice, user) {
        this.getImageAsBase64('assets/images/logo-invoice.png').subscribe((base64Image) => {
            const base64Data = base64Image.replace(/^data:image\/(png|jpg);base64,/, '');
            (error) => {
            };
            const doc = new jsPDF();
