...
export class WalletModel1AService {
  ...
  getBalanceInformationByAffiliateId(id: number) {
    return this.http
      .get<Response>(
        this.urlApi.concat(
          '/walletModel1A/GetBalanceInformationByAffiliateId/',
          id.toString()
        ), httpOptions
      )
      .pipe(
  ...
  payWithMyBalance(model: WalletRequest) {
    return this.http
      .post<Response>(this.urlApi.concat('/walletModel1A/payWithMyBalance1A'), model, httpOptions)
      .pipe(
        map((response) => {
          return response;
        })
